...
export class UserService {
  ...
  searchEmails(keyword: string) {
    return this.http.get<string[]>(
      `${environment.apiUrl}/api/chat/user/search-emails`,
      { params: { keyword } }
    );
  }
  ...
  getUsersForNewChat(userId: number): Observable<any> {
    ...
    return this.http.get<any>(`${environment.apiUrl}/api/chat/user/for-chat/${userId}`);
  }
  getUsersForNewChatForRoom(userId: number): Observable<ChatUserResponseDto[]> {
    console.log(userId);

    return this.http.get<ChatUserResponseDto[]>(`${environment.apiUrl}/api/chat/user/for-chat/room/${userId}`);
  }
}